...
mu = 0.006; # Friction coefficient in bearing
...
def Ebear(k): 
    return (mu*(avgF(k)*x)) + (((2*(0.5*k*(x**2)))/(alpha))/rTSpring)*Daxel*0.5*alpha*mu; # Energy loss due to internal forces in bearing (one way)
def Elost(k):
...
def EbearReturn(k, dh): 
    return (mu*(avgF(k)*x)) + (((2*(0.5*k*(x**2)))/(alpha))/rTSpring)*Daxel*alpha*mu; # Energy loss due to internal forces in bearing (one way)
def ElostFull(k):
...
def main():
    ...
    print("M/2 per rad:", kTors[0]*1000/2, "Nmm/rad,", kTors[quality-1]*1000/2, "Nmm/rad");
